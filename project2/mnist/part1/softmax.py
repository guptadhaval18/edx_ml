...
def compute_probabilities(X, theta, temp_parameter):
    ...
    #YOUR CODE HERE
    theta_x = theta @ X.T
    theta_x = theta_x / temp_parameter
    c = np.max(theta_x, axis=0)
    right = np.exp(theta_x - c)
    left = 1 / np.sum(right, axis=0)
    return left * right
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    return -sum / Y.shape[0] + lambda_factor * np.sum(np.square(theta)) / 2
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    non_regularized_gradient=np.dot(M-p,X)
